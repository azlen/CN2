<template>
    <div id="characterContainer" v-bind:class="{ fadeout: fadeOut, fullscreen: expandCharacter }" v-on:transitionend="startStrokeOrderAnimation">
		<svg ref="character" id="character" :class="`tone${character.tone}`" :style="{transform:`scale(${expand.scale.x}, ${expand.scale.y}) translate(${expand.translate.x}px, ${expand.translate.y}px)`}" v-on:click="tapCharacter" width="200px" height="200px" viewBox="-512 -512 1024 1024">
            <defs>
			...
			tapCharacter() {
				if (this.fadeOut) {
					...
					if (this.expandCharacter === true) {
						...
						this.expand.scale.y = (window.innerHeight + 30) / bbox.height;
						this.expand.translate.x = window.innerWidth / 2 - (bbox.x + bbox.width / 2);
						this.expand.translate.y = window.innerHeight / 2 - (bbox.y + bbox.height / 2) - 25;
						this.expand.translate.percx = this.expand.translate.x / window.innerWidth;
						this.expand.translate.percy = this.expand.translate.y / (window.innerHeight + 30);
					}
