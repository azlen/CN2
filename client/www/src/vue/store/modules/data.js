import Vue from 'vue'
import { distance, angle, createSVGMatrix, applyMatrix } from './../../../assets/js/util.js'
...
const state = {
  ...
  learnQueue: '的,在,日,是,手,我,口,和,月,有,也,他,不'.split(','),
  history: [],
  //currentExercise: {},
  /*characters: {
...
const getters = {
  currentWord(state) {
    console.log('GETTING WORD', state.learnQueue[0]);
    return state.learnQueue[0];
  },
  /*nextCharacter() {
...
// TODO: I SHOULD CREATE UTIL FUNCTION FOR APPLYING MATRICES TO POINTS
/*function fixPoint(p) {
...
// transformation matrix to fix offset of points
// later, this should be done on server and correct version should be sent
const fixedTransformationMatrix = createSVGMatrix({
  d: -1,
  e: -512,
  f: 400,
});
...
// processCharacter
// :: converts character data from server into more useful structured data for app
// characterData = character data received from server
function processCharacter(characterData) {
 ...
 // processed.tone = characterData.pinyinNumber.slice(-1);
  ...
  processed.medianPoints = characterData.medians.map(A => applyMatrix(A, fixedTransformationMatrix));
  processed.strokes = characterData.strokes.map(function(pathString) {
    ...
    while(i < pathItems.length - 1) {
      ...
      if(!isNaN(Number(a)) && !isNaN(Number(b))) {
        let fixed = applyMatrix([Number(a), Number(b)], fixedTransformationMatrix);
        newPathString += `${fixed[0]} ${fixed[1]} `;
  ...
  processed.medians = processed.medianPoints.map(function(A, index) {
...
const actions = {
  nextCharacter({ commit }) {
    commit('nextCharacter');
  },
  downloadCharacterData({ commit }, { words, type, callback }) {

    // TEMPORARY; REMOVE THIS
    commit('storeWordData', tempdata.map(processWord));
    callback();
    return;
    //
    ...
    console.log(type);
    Vue.http.get('http://192.168.0.14:3001', { params: parameters }).then(response => {
